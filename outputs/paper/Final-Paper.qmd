...
# Introduction
During the COVID-19 pandemic, we as a whole experienced 'lock downs.' If you are reading this you most likely experienced 'lock downs,' too, and it was during this time that working from home started to gain  popularity. What is easier forget, however, is that many of those who could not work from home lost their jobs. This report serves as a small reproduction of 'Work from Home before and after the COVID-19 Outbreak,' written by Alexander Bick, Adam Blandin, and Karel Mertens and published in the American Economic Review [@paper]. The original paper is an in depth analysis into home based work and employment loss specifically during February and May 2020. The goal of this paper, the estimand, is to look at the effect of the pandemic on working behavior.

The main findings of the original paper show the expected steep increase in unemployment as well as nuances in employment changes. For example, those working from home prior to the pandemic had few advantages post-pandemic, despite having previous experience in such an environment. Additionally, over 70& of those who worked from home did so effectively. The original paper is a well done analysis, however, this paper focuses on just a few of the aspects the original paper covers, namely unemployment, days worked per week, and days commutting to work per week. These three variables - unemployment, days worked per week, and days commuting to work per week - will be discussed in more detail in the Data section. 

During the pandemic, the transition from February to May saw many people in North America experience the need for 'lock downs.' This forced many people into very different work situations and these three variables serve as telling statistics as to the effect of this change and what these changes were. This is discussed more in the Results Section. The Discussion sections delves into the reasons for these employment changes, how the data supports these reasons, as well as comparisons to the original paper and the limitations of my work.


# Data
The data in this paper is downloaded from the replication package made available by the authors of the original paper, 'Work from Home before and after the COVID-19 Outbreak.' On the cover page is a DOI link which leads to the necessary files to download, (DOI: https://doi.org/10.1016/j.red.2022.11.002).

The entirety of this project was done using  the open source programming language R [@citeR]. In order to clean and analyse this data, I used the packages: haven [@haven], tidyverse [@tidyverse], and knitr [@knitr].

## The Data Sets
The authors have made available the raw data used in their paper, which includes 11 data sets regarding many labor statistics during the COVID-19 pandemic period. This paper only focuses on one, the RPS data set. The RPS is the Real-Time Population Survey and serves as a labor market survey meant to be similar to that of the Current Population Survey (CPS), a long running U.S. Bureau of Labor Statistics survey [@cps_rps]. The RPS began in March of 2020 as a weekly online questionnaire and continues today offered by the Federal Reserve Bank [@federal_reserve].  

It asks questions regarding general household information, income, and employment, and where it differs from the CPS is that it asks questions regarding time spent working and information on an individuals weekly commuting schedule. More specifically, it asks someone's employment status (as employed or unemployed), how many days and hours someone worked per week, and how many days they commuted to their job during February and May separately. It is important to note that these specific questions - how many days worked per week and how many days commutting to work - were not asked every week since this survey was so new in early 2020. 

In this paper we consider only the responses for the month of May 2020 in the RPS data set, where these questions regarding May and February employment were asked. 

## Cleaning the Data
Fortunately, the RPS is documented in an already clean, tidy, and easy-to-read manner since it is meant to provide labor statistics to the US Government at a higher frequency than the CPS [@federal_reserve]. The only cleaning required was that of selecting the variables to use for analysis out of the 61 offered. I selected information on an individual's employment status, days worked per week, and days commuted to work per week, for both February and May.

Next, I had to filter out the values that had irrelevant or no information in each of these areas. The end result is a data set with exactly 4,941 individuals with recorded employment status and relevant information on their weekly working behavior during the months of May and February. From this, I could then find the fraction of the days commuted to work per week. A small sample of this information is available in @tbl-samplerpsdata.  
...
# Results
The results of the analysis are in three parts. First, I attempted to replicate Table 1 of the original paper and did so successfully. @tbl-reptableone is the replicated table and shows the relationship between the RPS data as a whole in February versus March. It displays a large decrease in the employment rate from February to May among the surveyed by 30.4 log points. Additionally, despite the average number of workdays commutting decreasing by 35.7 log points, the average number of days working remained around the same as in February. 

```{r}
#| label: tbl-reptableone
#| tbl-cap: "This displays the difference in employment for the average individual in May compared to February 2020. This is also a replication of Table 1 from  'Work from Home before and after the COVID-19 Outbreak.' "
#| tbl-cap-location: bottom
#| echo: false
#| message: false
#| warning: false
...
Secondly, I attempted to recreate Table 2 pane a from the original paper and again did so successfully. @tbl-reptabletwo delves deeper into the commuting behaviors of each individual. It shows the percentage of those who commuted everyday, someday, or no days in May and February. In this table we are ignoring those that are unemployed as they do not commute to a job. The insights from @tbl-reptabletwo are discussed furtherin the Discussion section. 
...
df_rep2 |>
  kable()
```
Lastly, I attempted to replicate the original paper's Table 2 pane b. This proved more challenging and serves as a point of discussion later. The findings are not completely similar to that of the paper. @tbl-reptablethree shows the change in an individual's commuting behavior between February and March. The rows refer to the individuals work schedule in May, the columns refer to their schedule in February. In this table we consider all individuals who were employed in February, regardless of their employment status in May. 

```{r}
#| label: tbl-reptablethree
#| tbl-cap: "This table shows the relationship between an individuals change in commutting habits from February to May. The rows denote behavior in May, and the columns denote behavior in February. For example, 14.4% of individuals in the RPS that commuted each day in February now work from home each day in May."
#| tbl-cap-location: bottom
#| echo: false
#| message: false
#| warning: false
...
# Discussion
The discussion of this analysis will focus mainly on @tbl-reptablethree. First, @tbl-reptableone simply reassures the reader that the data collected does reflect that during the onset of COVID-19 people began working from home more and the unemployment rate did increase. The reasons for this are many and differ greatly depending on employment type and location. In particular, many of those who lost their job during the height of the lock downs would have been employed in jobs where they require person-to-person contact or communication, whether that be sales, waiting tables, or customer service. Therefore, it is not too far a stretch to connect the early COVID-19 measures with unemployment from February to May 2020. Further, in @tbl-reptableone we can see that the fraction of weekdays commuting to work had greatest changein log points whereas the average number of days working per week remained about the same. As for these two results, we can find explanation in that companies were encouraging and promoting their employees to work from home. It was a safety hazard and, as the original study has shown, productivity does not necessarily decrease due to working from home. In fact, in some case productivity increases [@work_from_home]. If productivity doesn't fall, with access to working from home, employees would be able to work a very similar amount to pre-pandemic measures in February.

Secondly, @tbl-reptabletwo focuses on the relationship between commuting to work considering pre and post pandemic measures. In this table, as explained in the Results section, we only consider those individuals who were employed in both February and March. The results give evidence that COVID-19 gave considerable rise to working from home. In February only 8.8% of employees were working from home whereas in May 33.3% of employees worked from home. 

Lastly, @tbl-reptablethree begins to look deeper into the individuals commuting behaviors relative to the pre-pandemic and post-pandemic measures. The table offers insights into how most people changed working 'styles' from February to May. For example, it displays the percentage of those who were working every day in February and are now working strictly from home. Or perhaps the individual is no longer employed. In this data set we consider a larger set of the RPS individuals, including some that do not have employment in May. Unfortunately, @tbl-reptablethree is not completely similar to that of the original paper's table 2 pane b I am attempting to replicate. Most notably, the final column, those that worked from home in February, should have seen inflated values as the other tables have shown working from home popularity, but @tbl-reptablethree does not show this. One takeaway that is similar to the original paper is that 14.4% of individuals in the RPS that commuted each day in February now work from home each day in May.

Possible reasons for this disparity brings to light the potential shortcomings of the analysis of this paper. For one, I focus only on the RPS data provided by the replication package attached to the original paper. There are several other data sets which may offer information to offer support to the original paper's conclusion, but I do not consider them. Another, is that the RPS has denoted a weighting to each individual based on their responses. In this way, the RPS may allow for every response to count for something, despite the individual possibly having no response for a variable at all. For example, if the individual does not record their employment type, it may be recorded as "-2" yet the original paper still uses this value in their calculation of employment rate, just that the weighting of this response will be much less to someone with a 'real' response. In my analysis I did not consider the weighting of each individual, instead I took their responses at face value. So, if an individual was unemployed/employed I counted their response and if they did not have a valid response for employment I did not include their data in my analysis.


# Conclusion
In conclusion, the original paper took a much more in-depth approach to the analysis of the working and commutting behavior of the individuals in the RPS. Consideration of the weighting of an individual's response is the largest difference between the original paper and this replication. Perhaps the differences in @tbl-reptablethree to the original paper's Table 2 pane b shows that the smaller size of the sample (4,941 total individuals surveyed) brings about the need for assigning weights, as opposed to omitting certain data points. 

The summary of the findings, however, is similar overall. We both found that employment decreased, a great deal more people began working from home in May 2020, and the amount of work required of the employees did not waver. The effect of COVID-19 on the work force could perhaps someday be seen as a pivotal moment in time, making employers rethink the effectiveness of an office as opposed to one's home. There is a great deal more to be learned from the pandemic's effect on the work force and this sample of only nearly 5,000 individuals shows there were noticeable effects. Lastly, this work could be used to examine the potential advantages and disadvantages of work place adjustments during future pandemic-like events.  
